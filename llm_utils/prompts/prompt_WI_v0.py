...
I will provide a mention with its context text and image information, as well as {num_cands} candidate entities with their descriptions. Based on all the available information, you need choose one entity that is most likely to correlate to the mention.'''
...
OUTPUT:
Based on the given information, the mention is "Governor Ed Rendell", the mention context text is… and the image information indicates that… We need to select one entity from the ten candidate entities that is most likely to correlate with the mention. Let's go through the steps to determine the correct entity:
